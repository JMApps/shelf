...
class PrayerTimeWidget : AppWidgetProvider() {
    ...
    companion object {
        ...
        const val ACTION_UPDATE = "individual.dmapps.shelf.ACTION_UPDATE_WIDGET"
        ...
        try {
            ...
            PRAYER_NAMES.forEach { prayerName ->
                try {
                    ...
                    val viewId =
                        context.resources.getIdentifier(prayerName, "id", context.packageName)
                    if (viewId != 0) {
                    ...
                    if (prayerDateTime.isBefore(now)) {
                        ...
                        if (nextPrayer == null || minutesRemaining < nextPrayer.minutesRemaining) {
                            nextPrayer =
                                NextPrayerInfo(prayerName, prayerDateTime, minutesRemaining)
                        }
                    }
                } catch (_: Exception) {
                }
